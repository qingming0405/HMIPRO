  ...
  <div class="my-dip-chart rdius box-shadow">
    ...
    >
      <div class="data-box select-none">
        <div class="data-title">操作</div>
        <ul class="operation-list">
          <li>
            <div class="operation-title">特征值</div>
            <div
          ...
          <li>
            <div class="operation-title">报警线</div>
        ...
        <ul class="data-list" v-show="data.clickView.isShow">
          <li v-for="(item, idx) in data.clickView.data" :key="idx">
            {{ item.name }}： {{ item.value }}
          </li>
        </ul>
      </div>
      <div class="chart-box">
        <div class="my-dip-chart" ref="myDipChart"></div>
      </div>
      <div class="none-data-box select-none radius" v-show="data.empty.isShow">
        <div>{{ data.empty.text }}</div>
      </div>
      <div class="pos-list" v-show="data.posListTab">
        <div
        ...
        >
          <span>测点列表</span>
          <div class="close-retrieval" @click="poslist(false)">
      ...
      <div class="search-data">
        <button @click="poslist(true)">测点选择</button>
        <button @click="compare">加入对比</button>
  ...
  methods: {
    ...
    setData(data, dipData, key) {
      ...
      if (length > 0) {
        ...
        let phase = [
          { filed: "rock_rms", name: "平均值方位", code: 41000 },
          { filed: "rock_p", name: "峰值方位", code: 42000 }
        ];
        if (loc === "2") {
          alarm = {
            40000: data[40000],
          ...
          phase = [{ filed: "rock_rms", name: "平均值方位", code: 41000 }];
        }
        ...
        setTimeout(() => {
          if (!dipData.empty.checkFlag) {
            ...
            dipData.empty.text = `${dipValue.posName}无数据`;
          }
        }, 500);
      }
    },
        ...
        toolTip: {
          formatter: function(params) {
            ...
            data.clickView = {
              isShow: true,
              data: [
                { name: "时间", value: getTime(time[index1][index2]) },
                {
                  name: data.currentEigen.uName,
                  value: `${x[index1][index2]}${unit}`
                },
                { name: "方位", value: `∠${y[index1][index2]}${unit}` }
