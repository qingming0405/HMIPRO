...
export default {
  ...
  methods: {
    changeColor(t_id, color) {
      let waveColorB = proConfig.waveColorB
      if (waveColorB.indexOf(t_id.toString()) !== -1) {
        color.series.line = '#000'
      } else {
        color.series.line = '#3D77FF'
      }
      return color
    },
    // 添加对比分析数据
    addCompare() {
      ...
      const data = cloneObj(this.chartData[currentKey], true)
      const wave = data.wave
      if (wave.srcY.length === 0) {
        this.$pop('当前没有波形数据')
        return
      }
      // 图谱数据
      ...
      wave.curX = wave.srcX
      wave.curY = wave.srcY
      wave.curUnitY = wave.srcUnitY
      spectrum.curX = spectrum.srcX
      spectrum.curY = spectrum.hanning
      spectrum.curUnitY = spectrum.srcUnitY
      spectrum.markLine.length = 0
      // 参数信息
      const params = this.paramsData[currentKey]
      const chartData = params.chartData
      const [macName, posName] = params.TitleName.split('-')
      const chart = {
        isShow: true,
        posName: params.pos.position_name,
        spectrumType: 'line',
        chartType: chartData.chartType,
        oneFreq: chartData.oneFreq,
        freq: chartData.freq,
        speed: chartData.speed,
        rms: chartData.rms,
        time: this.requestData.wave.currTime[0],
        spectral: chartData.spectral,
        samples: chartData.samples,
        posType: params.pos.position_type,
        p: chartData.p,
        pp: chartData.pp,
        wave,
        spectrum,
        macName: macName,
        pos: params.pos,
        flag: 1 /* 0: 拖入测点，1：通过波形加入对比波形*/,
      }
      const key = `${currentKey}_${chart.time}`
      const compareKeys = Object.keys(this.$store.state.compareData)
      if (compareKeys.includes(key)) {
        this.$pop('当前波形已在对比分析图中')
        return
      } else {
        // let [, , , , posType] = .split('_')
        ...
        this.$pop({
          btnNum: 2,
          content: '加入成功，是否打开对比波形图？',
        }).then((res) => {
          if (res) {
            const keys = `compare_${this.$store.state.count.compare}`
            this.$store.commit('addCount', 'compare')
  ...
  .my-wave-spectrum-chart {
    ...
    .screen-wave {
      ...
      .wave-spectrum-chart-4 {
        grid-template-rows: calc(46.5% - 5px) 46.5% 0;
