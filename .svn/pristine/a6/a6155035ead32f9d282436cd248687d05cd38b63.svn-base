...
export default {
  ...
  methods: {
    ...
    changeName(e) {
      ...
      let msg = this.$store.state[viewMsg.type]
      if (viewMsg.type !== 'tree') {
        msg = msg[viewMsg.id]
      }
      ...
      for (let i = 0, l = msg.length; i < l; i++) {
        const value = msg[i]
        text.push({
          val: value.mac_me || value.position_name || value.t_name,
          icon: '',
        })
      }
      this.$list({
        ...
        if (res.index > -1) {
          this.viewMsg.name = res.item.val
          this.changeMsg = msg[res.index]
        }
      })
    },
  },
