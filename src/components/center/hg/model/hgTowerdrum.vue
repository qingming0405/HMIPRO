<template>
  <!-- 塔筒 -->
  <div class="towerd-model">
    <div class="towerd-line1">
      <div class="towerd-line1-box1 Model-box">
        <div class="Model-header">{{towerDrumDataName[0]}}</div>
        <table class="model-table">
          <tbody class="my-table-tbody pro-scrollbar">
            <tr v-for="(item,index) in towerDrumData">
              <td
                class="model-td"
                :title="item.pos_name"
              >
      ...
      <div class="towerd-line1-box2 Model-box">
        <div class="Model-header">{{towerDrumDataName[1]}}</div>
        <table class="model-table">
          <tbody class="my-table-tbody pro-scrollbar">
            <tr
              v-for="(item,index) in basicsData"
              v-show="item.list_name === towerDrumDataName[1]"
            >
              <td
                class="model-td"
                :title="item.pos_name"
